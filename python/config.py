...
ракетных/авиаударах, терактах и т. п. Также исключи около-фронтовой контент: ситуации \
с правами человека в зонах конфликтов и оккупированных территориях, нападения на \
журналистов в зонах боевых действий, разрушение медиа-инфраструктуры в Газе/Украине, \
гуманитарные катастрофы и беженцы из зон войны. Модные тренды вроде military style, \
camouflage, combat boots, bomber jacket, cargo, khaki можно упоминать ТОЛЬКО как \
эстетическое направление в моде, без привязки к реальным военным конфликтам, армиям \
и оружию.
- НЕ ПУБЛИКУЙ новости без конкретики. КАЖДАЯ новость должна содержать имя конкретного \
дизайнера/CEO/директора-по-маркетингу/инфлюенсера ИЛИ название конкретного бренда/дома \
моды/коллекции/продукта/кампании ИЛИ конкретные цифры (год, проценты, суммы, \
ROI, охват). Формулировки «дизайнеры представили…», «бренды запустили…», «инфлюенсеры \
рассказали…», «мода становится более устойчивой» — БЕЗ имён, названий и цифр — \
НЕДОПУСТИМЫ.
- ОДИН ПУНКТ = ОДНА НОВОСТЬ. Не слепляй несколько разных не связанных одной темой \
новостей в один пункт.
- НЕ ПУБЛИКУЙ литературные эссе, биографические воспоминания, абстрактные \
размышления о моде/смысле/будущем без привязки к конкретному событию/коллекции/запуску.
...
AUDIO_INSTRUCTIONS = (
    ...
    "ситуации в зонах конфликтов и оккупированных территориях, гуманитарные катастрофы. "
    ...
    "Не пересказывайте новости без конкретики: каждая история должна содержать "
    "конкретные имена дизайнеров/CEO, названия брендов/коллекций/продуктов, цифры — "
    "без этого новость не упоминать."
)
